...
class ClusteringPage(QWidget):
    def __init__(self):
        super().__init__()
        self.df = None; self.feature_checkboxes = []; self.selected_features = []
        self.initUI()
    ...
    def initUI(self):
        ...
        file_group_box.setAlignment(Qt.AlignmentFlag.AlignRight) # <-- FIX for GroupBox title
        ...
        self.feature_group_box.setAlignment(Qt.AlignmentFlag.AlignRight) # <-- FIX for GroupBox title
        ...
        self.checkbox_layout.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignRight) # <-- FIX for CheckBox container
        ...
        middle_layout.addWidget(self.feature_group_box, stretch=1)

        # --- Step 4 ---
        comparison_group_box = QGroupBox("مرحله ۴: مقایسه نتایج (برای مشاهده جزئیات کلیک کنید)")
        comparison_group_box.setAlignment(Qt.AlignmentFlag.AlignRight) # <-- FIX for GroupBox title
        comparison_layout = QVBoxLayout()
        self.results_table = QTableView(); comparison_layout.addWidget(self.results_table)
        ...
        dmu_group_box.setAlignment(Qt.AlignmentFlag.AlignRight) # <-- FIX for GroupBox title
        dmu_layout = QVBoxLayout()
        self.dmu_table = QTableView(); dmu_layout.addWidget(self.dmu_table)
        dmu_group_box.setLayout(dmu_layout); main_layout.addWidget(dmu_group_box, stretch=1)
    ...
    def display_comparison_results(self, all_results):
        try: self.results_table.selectionModel().selectionChanged.disconnect(self.on_result_selection_changed)
        except TypeError: pass
        model = QStandardItemModel()
        model.setHorizontalHeaderLabels(["الگوریتم", "تعداد خوشه (k)", "امتیاز سیلوئت", "شاخص دیویس-بولدین"])
        for result in sorted(all_results, key=lambda x: x['silhouette'], reverse=True):
            ...
            for item in items:
                ...
                item.setTextAlignment(Qt.AlignmentFlag.AlignCenter) # <-- FIX for Table cell alignment
        ...
        self.results_table.selectionModel().selectionChanged.connect(self.on_result_selection_changed)
        if model.rowCount() > 0: self.results_table.selectRow(0)

    def populate_feature_checkboxes(self):
        for checkbox in self.feature_checkboxes: self.checkbox_layout.removeWidget(checkbox); checkbox.deleteLater()
        ...
        for col in all_columns:
            ...
            if col in numeric_columns: checkbox.setChecked(True)
    ...
    def on_result_selection_changed(self, selected, deselected):
        ...
        selected_row = selected.indexes()[0].row(); model = self.results_table.model()
        algorithm_name = model.item(selected_row, 0).text(); k = int(model.item(selected_row, 1).text())
        try:
            ...
            self.dmu_table.setModel(dmu_model); self.dmu_table.resizeColumnsToContents()
    ...
    def load_file_and_show_features(self):
        ...
        try:
            self.df = pd.read_excel(file_path); self.file_path_label.setText(file_path.split('/')[-1])
            self.populate_feature_checkboxes(); self.feature_group_box.setEnabled(True)
        except Exception as e: QMessageBox.critical(self, "خطا در خواندن فایل", f"فایل اکسل قابل پردازش نیست:\n{e}"); self.feature_group_box.setEnabled(False)

    def run_analysis(self):
        self.selected_features = [cb.text() for cb in self.feature_checkboxes if cb.isChecked()]
        if len(self.selected_features) < 2: QMessageBox.warning(self, "شاخص ناکافی", "لطفاً حداقل دو شاخص را برای تحلیل انتخاب کنید."); return
        try:
            QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
            all_results = get_all_clustering_results(self.df, self.selected_features)
            self.display_comparison_results(all_results)
        except Exception as e: traceback.print_exc(); QMessageBox.critical(self, "خطا در تحلیل", f"یک خطای پیش‌بینی نشده رخ داد:\n{e}")
        finally: QApplication.restoreOverrideCursor()
    ...
    def download_template(self):
        ...
        if save_path:
            try: df.to_excel(save_path, index=False); QMessageBox.information(self, "موفقیت", f"قالب با موفقیت در مسیر زیر ذخیره شد:\n{save_path}")
            except Exception as e: QMessageBox.critical(self, "خطا", f"خطا در ذخیره فایل:\n{e}")